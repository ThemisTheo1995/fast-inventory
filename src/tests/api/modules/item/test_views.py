...
def test_router_create_item(client, seed_workspace):
...
def test_router_get_item_not_found(client, seed_workspace):
    """Verifies looking up a non-existent item ID triggers a 404 error."""
    random_id = uuid.uuid4()
    response = client.get(f"/{seed_workspace}/items/{random_id}")
    assert response.status_code == status.HTTP_404_NOT_FOUND


def test_router_get_item_details(client, seed_workspace, active_item):
    """Verifies a user can fetch details of a valid item in their workspace."""
    response = client.get(f"/{seed_workspace}/items/{active_item.id}")
...
def test_router_get_items_list(client, seed_workspace, active_item):
    """Verifies fetching a paginated list of items for a workspace."""
    response = client.get(f"/{seed_workspace}/items")
    assert response.status_code == status.HTTP_200_OK
...
def test_router_get_items_search_and_pagination(client, seed_workspace, active_item):
    ...
    response = client.get(f"/{seed_workspace}/items", params={"search": search_term, "page": 1, "limit": 5})
    assert response.status_code == status.HTTP_200_OK
...
def test_router_patch_item(client, seed_workspace, active_item):
...
def test_router_delete_item(client, seed_workspace, active_item):
    """Verifies an item record can be successfully removed or soft-deleted."""
    response = client.delete(f"/{seed_workspace}/items/{active_item.id}")
    assert response.status_code == status.HTTP_204_NO_CONTENT


def test_router_item_tenant_isolation(client, alt_workspace, active_item):
    """
    ...
    response = client.get(f"/{alt_workspace}/items/{active_item.id}")
