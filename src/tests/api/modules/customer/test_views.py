...
def test_router_get_customer_not_found(client, seed_workspace):
    ...
    response = client.get(f"/{seed_workspace}/customers/{random_id}")
    assert response.status_code == status.HTTP_404_NOT_FOUND


def test_router_get_customer_details(client, seed_workspace, active_customer):
    """Verifies a user can fetch details of a valid customer in their workspace."""
    response = client.get(f"/{seed_workspace}/customers/{active_customer.id}")
...
def test_router_get_customers_list(client, seed_workspace, active_customer):
    """Verifies fetching a paginated list of customers for a workspace."""
    response = client.get(f"/{seed_workspace}/customers")
    assert response.status_code == status.HTTP_200_OK
...
def test_router_get_customers_search_and_pagination(client, seed_workspace, active_customer):
    ...
    response = client.get(f"/{seed_workspace}/customers", params={"search": search_term, "page": 1, "limit": 5})
    assert response.status_code == status.HTTP_200_OK
...
def test_router_customer_tenant_isolation(client, alt_workspace, active_customer):
    """
    ...
    response = client.get(f"/{alt_workspace}/customers/{active_customer.id}")
