...
def test_service_create_supplier_email_exists_raises(db_session, seed_workspace, active_supplier):
    ...
    with pytest.raises(SupplierEmailExistsError):
        service.create_supplier(workspace_id=seed_workspace, data=create_data)


def test_service_create_supplier_restores_soft_deleted(db_session, seed_workspace, active_supplier):
    ...
    service.delete_supplier(workspace_id=seed_workspace, supplier_id=original_id)
    ...
    restored_supplier = service.create_supplier(workspace_id=seed_workspace, data=create_data)
...
def test_service_update_supplier_email_exists_raises(db_session, seed_workspace, active_supplier):
    ...
    with pytest.raises(SupplierEmailExistsError):
        service.update_supplier(workspace_id=seed_workspace, supplier_id=second_supplier.id, data=update_data)


def test_service_update_supplier_remove_email(db_session, seed_workspace, active_supplier):
...
def test_service_update_supplier_same_email_allowed(db_session, seed_workspace, active_supplier):
...
def test_service_get_suppliers_pagination(db_session, seed_workspace, active_supplier):  # noqa
    """Verifies listing and paginating suppliers works correctly."""
    service = SupplierService(db=db_session)

    # Create a few extra suppliers (active_supplier is already 1)
    service.create_supplier(seed_workspace, SupplierCreate(name="Supplier A", email="a@test.com"))
    service.create_supplier(seed_workspace, SupplierCreate(name="Supplier B", email="b@test.com"))
    service.create_supplier(seed_workspace, SupplierCreate(name="Supplier C", email="c@test.com"))

    # Total should be 4. Let's fetch page 1, limit 2
    response = service.get_suppliers(workspace_id=seed_workspace, page=1, limit=2)

    assert response.total == 4
    assert len(response.items) == 2


def test_service_get_suppliers_search(db_session, seed_workspace):
    """Verifies the search parameter filters suppliers by name and email."""
    service = SupplierService(db=db_session)

    service.create_supplier(seed_workspace, SupplierCreate(name="Apple Supply", email="contact@apple.com"))
    service.create_supplier(seed_workspace, SupplierCreate(name="Banana Ltd", email="hello@bananas.com"))
    service.create_supplier(seed_workspace, SupplierCreate(name="Cherry Inc", email="apple@cherry.com"))

    # Search by Name
    res_name = service.get_suppliers(workspace_id=seed_workspace, search="Banana")
    ...
    res_multi = service.get_suppliers(workspace_id=seed_workspace, search="apple")
    assert res_multi.total == 2

    # Search should be case-insensitive
    res_case = service.get_suppliers(workspace_id=seed_workspace, search="CHERRY")
