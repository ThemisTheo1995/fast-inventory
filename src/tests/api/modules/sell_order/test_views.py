...
async def test_router_create_sell_order_success(client, seed_workspace, active_customer):
    ...
    response = await client.post(f"/{seed_workspace}/sell-orders", json=payload)
...
@pytest.mark.asyncio
async def test_router_create_sell_order_empty_lines(client, seed_workspace):
...
@pytest.mark.asyncio
async def test_router_create_sell_order_validation_error(client, seed_workspace):
...
@pytest.mark.asyncio
async def test_router_get_sell_orders_success(client, seed_workspace, active_sell_order):
    """Verifies fetching the paginated list of sell orders returns 200 OK."""
    response = await client.get(f"/{seed_workspace}/sell-orders")
...
@pytest.mark.asyncio
async def test_router_get_sell_orders_with_pagination_and_search(client, seed_workspace):
    """Verifies pagination and search query parameters bind correctly to the endpoint."""
    response = await client.get(f"/{seed_workspace}/sell-orders?search=SO-TEST&page=2&limit=5")
...
@pytest.mark.asyncio
async def test_router_get_sell_orders_invalid_pagination(client, seed_workspace):
    """Verifies that invalid pagination types trigger 422."""
    response = await client.get(f"/{seed_workspace}/sell-orders?page=not-a-number")
    assert response.status_code == status.HTTP_422_UNPROCESSABLE_CONTENT
...
@pytest.mark.asyncio
async def test_router_get_sell_order_success(client, seed_workspace, active_sell_order):
    """Verifies fetching an existing sell order by ID returns 200 OK."""
    response = await client.get(f"/{seed_workspace}/sell-orders/{active_sell_order.id}")
...
@pytest.mark.asyncio
async def test_router_get_sell_order_not_found(client, seed_workspace):
    """Verifies fetching a non-existent sell order returns 404 Not Found."""
    fake_id = uuid.uuid4()
    response = await client.get(f"/{seed_workspace}/sell-orders/{fake_id}")
    assert response.status_code == status.HTTP_404_NOT_FOUND


@pytest.mark.asyncio
async def test_router_get_sell_order_invalid_uuid(client, seed_workspace):
    """Verifies passing a malformed UUID in the path returns 422 Unprocessable Content."""
    response = await client.get(f"/{seed_workspace}/sell-orders/this-is-not-a-uuid")
    assert response.status_code == status.HTTP_422_UNPROCESSABLE_CONTENT
...
@pytest.mark.asyncio
async def test_router_patch_sell_order_not_found(client, seed_workspace):
    """Verifies patching a non-existent sell order returns 404 Not Found."""
    payload = {"so_number": "SO-NEW-NAME"}
    response = await client.patch(f"/{seed_workspace}/sell-orders/{uuid.uuid4()}", json=payload)
    assert response.status_code == status.HTTP_404_NOT_FOUND


@pytest.mark.asyncio
async def test_router_patch_sell_order_validation_error(client, seed_workspace, active_sell_order):
...
@pytest.mark.asyncio
async def test_router_delete_sell_order_success(client, seed_workspace, active_sell_order):
    """Verifies deleting an existing sell order returns 204 No Content."""
    response = await client.delete(f"/{seed_workspace}/sell-orders/{active_sell_order.id}")
    assert response.status_code == status.HTTP_204_NO_CONTENT

    # Re-verify fetching directly returns a 404
    get_resp = await client.get(f"/{seed_workspace}/sell-orders/{active_sell_order.id}")
    assert get_resp.status_code == status.HTTP_404_NOT_FOUND


@pytest.mark.asyncio
async def test_router_delete_sell_order_not_found(client, seed_workspace):
    """Verifies attempting to delete a non-existent sell order returns 404 Not Found."""
    response = await client.delete(f"/{seed_workspace}/sell-orders/{uuid.uuid4()}")
    assert response.status_code == status.HTTP_404_NOT_FOUND
...
@pytest.mark.asyncio
async def test_router_add_sell_order_line_success(client, seed_workspace, active_sell_order):
    """Verifies adding a line to an existing sell order returns 201 Created."""
    payload = {"item_id": None, "quantity": 3, "unit_cost": 500}

    response = await client.post(f"/{seed_workspace}/sell-orders/{active_sell_order.id}/lines", json=payload)
...
@pytest.mark.asyncio
async def test_router_add_sell_order_line_so_not_found(client, seed_workspace):
    """Verifies adding a line to a non-existent sell order returns 404 Not Found."""
    payload = {"quantity": 1, "unit_cost": 100}
    response = await client.post(f"/{seed_workspace}/sell-orders/{uuid.uuid4()}/lines", json=payload)
    assert response.status_code == status.HTTP_404_NOT_FOUND


@pytest.mark.asyncio
async def test_router_add_sell_order_line_invalid_data(client, seed_workspace, active_sell_order):
...
@pytest.mark.asyncio
async def test_router_update_sell_order_line_success(client, seed_workspace, active_sell_order, active_sell_order_line):
...
@pytest.mark.asyncio
async def test_router_update_sell_order_line_not_found(client, seed_workspace, active_sell_order):
    """Verifies patching a non-existent line ID returns 404 Not Found."""
    payload = {"quantity": 10, "unit_cost": 10}
    response = await client.patch(
        f"/{seed_workspace}/sell-orders/{active_sell_order.id}/lines/{uuid.uuid4()}", json=payload
    )
    assert response.status_code == status.HTTP_404_NOT_FOUND


@pytest.mark.asyncio
async def test_router_update_sell_order_line_wrong_parent(client, seed_workspace, active_sell_order_line):
...
@pytest.mark.asyncio
async def test_router_delete_sell_order_line_success(client, seed_workspace, active_sell_order, active_sell_order_line):
    """Verifies deleting an existing line returns 204 No Content."""
    response = await client.delete(
        f"/{seed_workspace}/sell-orders/{active_sell_order.id}/lines/{active_sell_order_line.id}"
    )
    assert response.status_code == status.HTTP_204_NO_CONTENT
...
@pytest.mark.asyncio
async def test_router_delete_sell_order_line_not_found(client, seed_workspace, active_sell_order):
    """Verifies attempting to delete a non-existent line returns 404 Not Found."""
    response = await client.delete(f"/{seed_workspace}/sell-orders/{active_sell_order.id}/lines/{uuid.uuid4()}")
    assert response.status_code == status.HTTP_404_NOT_FOUND
