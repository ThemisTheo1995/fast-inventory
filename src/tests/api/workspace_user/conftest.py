...
def target_user(db_session) -> User:
    ...
    user = User(id=uuid.uuid4(), email="target-member@company.com", first_name="Target", last_name="Member")
    db_session.add(user)
    db_session.commit()
    return user
...
@pytest.fixture
def target_workspace_user(db_session, seed_workspace, target_user) -> WorkspaceUser:
