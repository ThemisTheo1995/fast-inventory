...
async def update_me(
    request: Request, data: UserUpdateRequest, db: Annotated[AsyncSession, Depends(get_db)]
) -> UserResponse:
    workspace_user = request.state.workspace_user
    current_user = workspace_user.user

    updated_user = await WorkspaceUserService(db).update_user(
        current_user,
        data,
    )
...
async def get_workspace_user(
    workspace_user_id: UUID, db: Annotated[AsyncSession, Depends(get_db)]
) -> WorkspaceUserResponse:
    service = WorkspaceUserService(db)
...
@router.patch(
    "/workspace-users/{workspace_user_id}",
    response_model=WorkspaceUserResponse,
...
async def update_workspace_user(
    ...
    workspace_user = request.state.workspace_user
    service = WorkspaceUserService(db)
    return await service.update_workspace_user(data=data, target_id=workspace_user_id, actor=workspace_user)
